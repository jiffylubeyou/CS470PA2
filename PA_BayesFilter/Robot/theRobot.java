...
public class theRobot extends JFrame {
    ...
    // variables to store information entered through the command-line about the
    // current scenario
    boolean isManual = false; // determines whether you (manual) or the AI (automatic) controls the robots
                              // movements
    ...
    // 4 -- whether it's initial position is known. if known, its position is stored
    // in (startX, startY)
    ...
    void initializeProbabilities() {
        ...
        } else { // otherwise, set up a uniform prior over all the positions in the world that
                 // are open spaces
            int count = 0;
    ...
    // TODO: update the probabilities of where the AI thinks it is based on the
    // action selected and the new sonar readings
    ...
    // For example, the sonar string 1001, specifies that the sonars found a wall in
    // the North and West directions, but not in the South and East directions
    void updateProbabilities(int action, String sonars) {
        ...
        for (int y = 1; y < mundo.height - 1; y++) {
            for (int x = 1; x < mundo.width - 1; x++) {
                ...
                double otherMoveProb = (1 - moveProb) / 4;
                for (int i = 0; i < 5; i++) {
                ...
                newprobs[x][y] = (probs[x][y + 1] * action_prob[0]) + (probs[x][y - 1] * action_prob[1]) +
                        (probs[x - 1][y] * action_prob[2]) + (probs[x + 1][y] * action_prob[3])
                        + (probs[x][y] * action_prob[4]);
    ...
    // This is the function you'd need to write to make the robot move using your
    // AI;
    ...
    void doStuff() {
        ...
        while (true) {
            try {
                ...
                updateProbabilities(action, sonars); // TODO: this function should update the probabilities of where the
                                                     // AI thinks it is
                ...
                Thread.sleep(decisionDelay); // delay that is useful to see what is happening when the AI selects
                                             // actions
                                             // decisionDelay is specified by the send command-line argument, which is
